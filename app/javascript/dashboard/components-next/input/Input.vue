...
const inputOutlineClass = computed(() => {
  switch (props.messageType) {
    ...
    default:
      return 'outline-n-weak dark:outline-n-weak hover:outline-n-strong dark:hover:outline-n-strong disabled:outline-n-weak dark:disabled:outline-n-weak focus:outline-n-brand dark:focus:outline-n-brand focus:ring-4 focus:ring-n-brand/20';
  }
});
    ...
    <input
      ...
      :max="
        ['date', 'datetime-local', 'time', 'number'].includes(type)
          ...
          : undefined
      "
      class="block w-full reset-base text-sm !mb-0 outline outline-[1.5px] border-none border-0 outline-offset-[-1px] rounded-xl bg-n-alpha-black2 shadow-soft file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-n-slate-10 dark:placeholder:text-n-slate-10 disabled:cursor-not-allowed disabled:opacity-50 text-n-slate-12 transition-all duration-150 ease-out [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
      @input="handleInput"
      @focus="handleFocus"
      @blur="handleBlur"
